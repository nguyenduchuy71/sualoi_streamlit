...
class Vocab():
    ...
    def encode(self, chars):
        return [self.start] + [self.c2i[c] for c in chars] + [self.end]

    def decode(self, ids):
        ids = [i for i in ids]
        first = 1 if self.start in ids else 0
        last = ids.index(self.end) if self.end in ids else None
    ...
    def decode_split(self, ids):
        ...
        first = 1 if self.start in ids else 0
        last = ids.index(self.end) if self.end in ids else None
        sent = [self.i2c[i] for i in ids[first:last]]
        return sent
...
class Encoder(tf.keras.layers.Layer):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
    ...
    # self.lstm = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(d_model, return_sequences=True))
...
def predict(inp_sentence):
  s = preprocessing_data(inp_sentence)  
  inp_sentence = vocab.encode(s)
  
  encoder_input = tf.expand_dims(inp_sentence, 0)
  encoder_input = pad_sequences(encoder_input, maxlen=MAXLEN, padding='post')
...
def sualoi(sentence):
  list_ngrams=[]
  for i in gen_ngrams(sentence, 5):
    if len(" ".join(i)) < MAXLEN:
      list_ngrams.append(" ".join(i))
  result=[]
  for i in list_ngrams:
    pred = predict(str(i))
    result.append(vocab.decode(pred.numpy()))
  sen=''
  for i in result[:-1]:
    sen+=i.split()[0]+' '
  sen+=result[-1]
